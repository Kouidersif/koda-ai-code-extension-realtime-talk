...
const geminiClient = new GeminiClient({
  onOpen: () => {
    ...
    geminiClient.sendText(
      `System: Say Hi concisely to acknowledge connection.`
    );
  },
  onMessage: (event) => {
...
function handleJsonMessage(msg) {
  if (msg.type === "interrupted") {
    ...
    // Skip Gemini's user transcription - we'll use OpenAI's instead
    // (keep this for backwards compatibility but don't display)
  ...
  } else if (msg.type === "openai_transcription") {
    // Update transcription panel
    const data = msg.data;
    document.getElementById("transcription-text").textContent = data.transcription || "N/A";
    document.getElementById("translation-text").textContent = data.translation || "N/A";
    document.getElementById("target-lang").textContent = data.target_lang || "unknown";
    
    // Add visual feedback to transcription panel
    const transcriptionBox = document.getElementById("transcription-box");
    transcriptionBox.style.backgroundColor = "#e8f5e9";
    setTimeout(() => {
      transcriptionBox.style.backgroundColor = "";
    }, 500);
    
    // Display in chat log as user message
    const userMessage = data.transcription || "";
    const translation = data.translation || "";
    const lang = data.target_lang || "unknown";
    
    // Create user message with transcription and translation
    ...
    // Add translation if different from transcription
    if (translation !== userMessage && translation) {
      const translationDiv = document.createElement("div");
      translationDiv.style.fontSize = "0.85em";
      translationDiv.style.color = "#666";
